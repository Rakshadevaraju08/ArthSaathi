...
export default function LedgerScreen() {
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data)) {
        // Normalize date field from ISO string to YYYY-MM-DD
      ...
      console.warn('[Ledger] Failed to fetch transactions:', error);
    }
  }, [setTransactions]);
  ...
  // ── Submit new transaction to backend ────────────────
  const submitTransaction = async () => {
    ...
    try {
      ...
      setShowAdd(false);
      // Refresh from backend to get the server-generated ID and timestamp
      await fetchTransactions();
    } catch (error) {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }} edges={['top']}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#f1f5f9' }}>
        <View>
          ...
          <Text style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>Synced with server</Text>
        </View>
        <TouchableOpacity
          onPress={() => {
      ...
      <BottomSheet visible={showAdd} onClose={() => setShowAdd(false)}>
        ...
        <TouchableOpacity onPress={submitTransaction} disabled={submitting}>
          <LinearGradient colors={[C.emerald500, C.teal600]} style={{ borderRadius: 14, paddingVertical: 15, alignItems: 'center', opacity: submitting ? 0.7 : 1 }}>
